...
export function EventsList({ 
  visibleCalendars: externalVisibleCalendars, 
  onVisibleCalendarsChange,
  showFilters = true
}: EventsListProps) {
  const { events, calendars, addEvent, updateEvent, deleteEvent } = useEvents();
  const [internalSelectedCalendars, setInternalSelectedCalendars] = useState<string[]>(calendars.map(c => c.id));
  
  const selectedCalendars = externalVisibleCalendars || internalSelectedCalendars;
  const setSelectedCalendars = onVisibleCalendarsChange || setInternalSelectedCalendars;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        {/* Mobile View (Cards) */}
        <div className="md:hidden flex flex-col gap-2 p-4 bg-slate-50/50">
